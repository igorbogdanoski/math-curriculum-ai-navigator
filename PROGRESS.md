...
| 12 | `pending` | Фаза 11 | AI Content Recovery (SDK fix, model fix) |
| 13 | `pending` | Фаза 12 | Security Hardening (Firebase Auth proxy) & Accessibility |

---

## 🛠️ Детално за последните фази

### Фаза 11 — AI Content Recovery
- **Проблем**: Апликацијата престана да генерира содржина поради невалиден модел и неточна SDK имплементација.
- **Решение**:
  - Стандардизиран модел `gemini-1.5-flash` низ целиот систем.
  - Поправен `GoogleGenAI` конструктор во API рутите.
  - Реструктуиран `contents` пејлоуд во правилен низа-формат `[{ parts: [...] }]`.
  - Отстранет `thinkingConfig` за flash модели (неподдржано).

### Фаза 12 — Security & Accessibility
- **API Authentication**: Овозможена задолжителна Firebase ID token верификација на серверските функции преку Firebase Admin SDK.
- **Focus Trapping**: Интегриран `ModalContainer` во `ModalManager` за WCAG усогласеност (Escape клуч, заклучување на скрол, враќање на фокус).
- **SRI Stability**: Поправени KaTeX хешовите во `index.html` за стабилно вчитување на математичките формули.
- **Firebase Admin Fix**: Трансформиран `FIREBASE_SERVICE_ACCOUNT` од код во правилен JSON формат.
