...
export const SharedPlanView: React.FC<SharedPlanViewProps> = ({ data }) => {
  ...
  if (!plan) {
    return (
      <div className="p-8 text-center">
        ...
        <button type="button" onClick={() => navigate('/')} className="mt-4 px-4 py-2 bg-brand-primary text-white rounded">
          Врати се на почетна
        </button>
      </div>
    );
  ...
  const getImportLabel = () => {
    if (importSuccess) return <><ICONS.check className="w-5 h-5 mr-2" /> Успешно увезено!</>;
    if (isImporting)   return <><ICONS.spinner className="animate-spin w-5 h-5 mr-2" /> Се увезува...</>;
    if (!isAuthenticated) return <><ICONS.plus className="w-5 h-5 mr-2" /> Пријави се за да увезеш</>;
    return <><ICONS.plus className="w-5 h-5 mr-2" /> Увези во мојата библиотека</>;
  };
