  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-black text-brand-primary mb-1">Адаптивна практика</h1>
        <p className="text-gray-500">Вежбај матурски прашања по тема и следи го напредокот</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 space-y-6">
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Теми</h3>
            <button
              type="button"
          ...
          <p className="text-xs text-gray-400 mt-2">Ниту едно избрано = сите теми</p>
        </section>

        {/* Part filter */}
        <section>
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Дел</h3>
          <div className="flex gap-2">
            {[
              { v: 1, label: 'Дел I — MC (1pt)'    },
              { v: 2, label: 'Дел II — Отворено (2pt)'  },
              { v: 3, label: 'Дел III — Отворено (3-5pt)' },
            ].map(({ v, label }) => (
              <button
                key={v} type="button"
                onClick={() => setParts(prev => toggle(prev, v))}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                  parts.includes(v) ? 'bg-brand-primary text-white border-brand-primary' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'
                }`}
              >
                {label}
              </button>
        ...
        {/* DoK */}
        <section>
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Ниво на тежина (DoK)</h3>
          <div className="flex gap-2">
            {[1,2,3,4].map(l => (
              <button
                key={l} type="button"
                onClick={() => setDok(prev => toggle(prev, l))}
                className={`w-10 h-10 rounded-lg text-sm font-black border transition-all ${
                  dokLevels.includes(l) ? 'bg-brand-primary text-white border-brand-primary' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'
                }`}
              >
                {l}
              </button>
        ...
        <section className="flex items-center gap-6">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              type="checkbox" checked={doShuffle}
              onChange={e => setShuffle(e.target.checked)}
              className="w-4 h-4 accent-brand-primary"
            />
            <span className="text-sm font-medium text-gray-700">Мешај ред</span>
          </label>
        ...
        <div className="pt-2">
          <button
            ...
            className={`w-full py-3 rounded-xl font-black text-lg transition-all ${
              canStart
                ? 'bg-brand-primary text-white shadow-lg hover:shadow-xl hover:-translate-y-0.5'
                : 'bg-gray-100 text-gray-400 cursor-not-allowed'
            }`}
          >
            {canStart ? `Започни — ${previewCount} прашања` : 'Нема прашања за избраните филтри'}
          </button>
        </div>
      </div>
    </div>
