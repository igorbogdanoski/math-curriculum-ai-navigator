...
import { MATH_STANDARDS, CROSS_CURRICULAR_WITH_MATH, AREA_LABELS, AREA_ICONS, AREA_COLORS } from '../../data/allNationalStandardsComplete';
...
/** Keyword match: does this plan topic address this standard's description? */
function topicsAddressStd(planTopics: string[], description: string): boolean {
...
const NationalStandardsPanel: React.FC<NationalStandardsPanelProps> = ({ planTopics, gradeNum }) => {
  ...
  // ── Math standards (III-А.1–27): deduplicate by code from national-standards.ts
  ...
  // ── Cross-curricular: group by area
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-4">
      ...
      {/* ── Section A: Math standards grid (III-А.1–27) ── */}
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-bold text-indigo-700">
            🔢 III. Математика — III-А.1–27
          </p>
      ...
      {/* ── Divider + cross-curricular toggle ── */}
      <button
        type="button"
      ...
      {showCross && (
        <div className="space-y-2">
          <p className="text-[10px] text-gray-400">
            ...
            Кликни на подрачјето за детали.
          </p>
          {areasOrder.map(area => {
