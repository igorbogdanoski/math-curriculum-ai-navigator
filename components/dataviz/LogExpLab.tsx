...
function generateLogExpSet(difficulty: 1 | 2 | 3, count = 6): LabExercise[] {
  const pool = difficulty === 1 ? LOGEXP_POOL1 : difficulty === 2 ? LOGEXP_POOL2 : LOGEXP_POOL3;
  return Array.from({ length: count }, (_, i) => ({ id: `logexp-${difficulty}-${i}`, ...pool[i % pool.length] }));
}
...
function LogExpExercisesTab() {
  ...
  const loadSet = useCallback((d?: 1 | 2 | 3) => {
    ...
    loadExercises(generateLogExpSet(level));
...
// ─── Main export ──────────────────────────────────────────────────────────────
type LogExpTab = 'logexp' | 'prob' | 'exercises';
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 flex-wrap text-xs font-bold">
        {([['logexp', 'Лог / Експ'], ['prob', 'Веројатност'], ['exercises', '✏️ Вежбај']] as const).map(([id, lbl]) => (
      ...
      {logExpTab === 'exercises' && <LogExpExercisesTab />}
    </div>
  );
}
