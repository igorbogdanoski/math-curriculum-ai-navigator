...
import { SecondaryStatsLab } from './SecondaryStatsLab';
...
function renderLab(lang: Language) {
  ...
  return render(<LanguageProvider><SecondaryStatsLab /></LanguageProvider>);
}

describe('SecondaryStatsLab i18n smoke test', () => {
  beforeEach(() => { cleanup(); localStorage.clear(); });

  (['mk', 'en'] as Language[]).forEach(lang => {
    it(`renders every non-exercise tab without leaking raw i18n keys (${lang})`, () => {
      renderLab(lang);
      const NON_EXERCISE_TAB_COUNT = 5; // normal, regression, bayes, montecarlo, chisq (exercises tab excluded)
