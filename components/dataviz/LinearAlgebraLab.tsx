...
type NxNMethod = 'gauss' | 'cramer' | 'cofactor' | 'lu' | 'adj';
...
function NxNSolverLab() {
  ...
  const availMethods = MATRIX_METHODS.filter(m => m.mode.includes(mode));
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-sky-50 border border-sky-100 rounded-xl p-3 text-xs text-sky-700">
        <strong>Методи:</strong> Gauss — универзален · Cramer — нагледен за мали n · Cofactor — рекурзивен Лаплас ·
        LU — ефикасен за повеќе b-вектори · Adj/det — инверз аналитички
      </div>
    </div>
  );
}
...
// ─── Main export ──────────────────────────────────────────────────────────────
type LinAlgTab = 'matrices' | 'vectors' | 'transforms' | 'systems' | 'nxn';
