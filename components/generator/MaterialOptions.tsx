...
import { QuestionType, type DifferentiationLevel, type TeachingProfile } from '../../types';
...
const ScenarioOptions: React.FC<Pick<MaterialOptionsProps, 'state' | 'dispatch'>> = ({ state, dispatch }) => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
        <div>
            ...
            <select id="activity-focus" value={state.activityFocus} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'activityFocus', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md">
        ...
        <div>
            ...
            <select id="scenario-tone" value={state.scenarioTone} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'scenarioTone', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md">
        ...
        <div>
            ...
            <select id="learning-design" value={state.learningDesignModel} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'learningDesignModel', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md">
...
const AssessmentOptions: React.FC<MaterialOptionsProps> = ({ state, dispatch, user }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
        ...
        <div><label htmlFor="numQuestions" className="block text-sm font-medium text-gray-700">Број на прашања</label><input id="numQuestions" type="number" value={state.numQuestions} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'numQuestions', value: Math.max(1, parseInt(e.target.value) || 1)}})} min="1" max="20" className="mt-1 block w-full p-2 border-gray-300 rounded-md" /></div>
             ...
             <div className="relative flex items-start">
                <div className="flex items-center h-5">
                    <input
                        ...
                        onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'includeSelfAssessment', value: e.target.checked }})}
...
const ExitTicketOptions: React.FC<Pick<MaterialOptionsProps, 'state' | 'dispatch'>> = ({ state, dispatch }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
        <div>
            ...
            <input id="exit-ticket-q-count" type="number" value={state.exitTicketQuestions} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'exitTicketQuestions', value: Math.max(1, Math.min(3, parseInt(e.target.value) || 1)) }})} min="1" max="3" className="mt-1 block w-full p-2 border-gray-300 rounded-md" />
        </div>
        <div>
            <label htmlFor="exit-ticket-focus" className="block text-sm font-medium text-gray-700">Фокус на прашањата</label>
            <select id="exit-ticket-focus" value={state.exitTicketFocus} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'exitTicketFocus', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md">
...
const RubricOptions: React.FC<Pick<MaterialOptionsProps, 'state' | 'dispatch'>> = ({ state, dispatch }) => (
    <div className="space-y-4 my-4 animate-fade-in">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label htmlFor="activityTitle" className="block text-sm font-medium text-gray-700">Наслов на активноста за рубриката</label><input id="activityTitle" value={state.activityTitle} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'activityTitle', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md" placeholder="пр. Проект: Питагорова теорема"/></div>
            <div><label htmlFor="activityType" className="block text-sm font-medium text-gray-700">Тип на активност</label><select id="activityType" value={state.activityType} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'activityType', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md"><option>Проект</option><option>Тест</option><option>Работен лист</option><option>Квиз</option><option>Домашна работа</option></select></div>
        </div>
        <div><label htmlFor="criteriaHints" className="block text-sm font-medium text-gray-700">Клучни критериуми (опционално, одделени со запирка)</label><input id="criteriaHints" value={state.criteriaHints} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'criteriaHints', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md" placeholder="пр. Точност, Креативност, Презентација"/></div>
    </div>
);

const IllustrationOptions: React.FC<Pick<MaterialOptionsProps, 'state' | 'dispatch'>> = ({ state, dispatch }) => (
    <div className="my-4 animate-fade-in">
        <div><label htmlFor="illustrationPrompt" className="block text-sm font-medium text-gray-700">Опис за илустрацијата</label><textarea id="illustrationPrompt" value={state.illustrationPrompt} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'illustrationPrompt', value: e.target.value }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md" placeholder="пр. Визуелен доказ за Питагорова теорема со квадрати над страните на триаголник" rows={3}></textarea><p className="text-xs text-gray-500 mt-1">Опишете ја визуелната идеја. Ако сте прикачиле слика, опишете какви промени сакате да направите на неа.</p></div>
    </div>
);
...
const LearningPathOptions: React.FC<MaterialOptionsProps> = ({ state, dispatch, user }) => (
    <div className="animate-fade-in">
        ...
        <select id="student-profiles-lp" multiple value={state.selectedStudentProfileIds} onChange={(e) => dispatch({ type: 'SET_FIELD', payload: { field: 'selectedStudentProfileIds', value: Array.from(e.target.selectedOptions, (o: HTMLOptionElement) => o.value) }})} className="mt-1 block w-full p-2 border-gray-300 rounded-md h-24">
            {user?.studentProfiles?.length ? (
                user.studentProfiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)
            ) : (
