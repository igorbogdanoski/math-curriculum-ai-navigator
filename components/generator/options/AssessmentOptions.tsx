...
import { QuestionType, type DifferentiationLevel, type StudentProfile } from '../../../types';
import { ICONS } from '../../../constants';
import type { MaterialOptionsProps } from './materialOptionsProps';
import { BLOOM_LEVELS } from './materialOptionsProps';

export const AssessmentOptions: React.FC<MaterialOptionsProps> = ({ state, dispatch, user }) => {
                                ...
                                <div className="animate-fade-in flex flex-col gap-3">
                                    {(state.materialType === 'ASSESSMENT' || state.materialType === 'QUIZ') && (
                                        <label className={`cursor-pointer flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-sm font-bold transition-all w-fit ${state.generateAllLevels ? 'bg-gradient-to-r from-indigo-500 to-violet-500 border-indigo-400 text-white shadow-sm' : 'bg-white border-gray-200 text-gray-700 hover:bg-gray-50'}`}>
                                            <input type="checkbox" checked={!!state.generateAllLevels} onChange={e => dispatch({ type: 'SET_FIELD', payload: { field: 'generateAllLevels', value: e.target.checked } })} className="sr-only" />
                                    ...
                                    return (
                                        <button key={key} type="button"
                                            onClick={() => {
                                                ...
                                                if (newDist[key] !== undefined) delete newDist[key]; else newDist[key] = 1;
