...
- СЛЕДЕЊЕ: Конкретни методи (набљудување, прашања/одговори, врстничка евалуација).
`;
...
async parseScenarioFromText(rawText: string, profile?: TeachingProfile): Promise<Partial<LessonPlan>> {
      const safe = sanitizePromptInput(rawText, 16000);
      const truncated = safe.length > 13000;
      const excerpt = safe.slice(0, 13000);
      const prompt = `
### УЛОГА
...
${truncated ? '\n⚠️ Напомена: Текстот е делумно искратен — работиш со главниот дел на документот.\n' : ''}
...
2. Идентификувај цели на часот. За секоја цел одреди bloomsLevel (еден од: Remembering/Understanding/Applying/Analyzing/Evaluating/Creating) врз основа на глаголот (знае→Remembering, разбира→Understanding, примени→Applying, анализира→Analyzing, оцени→Evaluating, создаде→Creating).
3. Подели активноста на: Воведна / Главни активности (низа) / Завршна. За секоја главна активност одреди bloomsLevel.
4. Ако постои ДИФЕРЕНЦИЈАЦИЈА (адаптација за различни нивоа ученици, поддршка, проширување) — извлечи ја дословно во полето differentiation.
5. Извлечи материјали/наставни средства ако се споменати.
6. Ако одделение или тема НЕ се спомнати — остави полето празно. НЕ претпоставувај.
7. assessmentStandards: ако во текстот се наведени ИСПИСАНИ критериуми за оценување или стандарди (пр. "Решава задачи...", "Применува...") — извлечи ги дословно (максимум 6). Ако нема исписани стандарди — остави празна низа.
8. originalAuthor: ако во документот е наведено "Изготвил/-а:", "Автор:", "Наставник:" или слично — извлечи го целосното ime. Ако нема — празен стринг.
9. originalSchool: ако во документот е наведено училиште/ОУ/СОУ/установа — извлечи го целосниот назив. Ако нема — празен стринг.

ВАЖНО: Ова е ТРАНСКРИПЦИЈА на постоечка содржина — НЕ создавај нова. Празно поле е подобро од измислена содржина.
`;
      const schema = {
          type: Type.OBJECT,
          properties: {
              ...
              originalSchool: { type: Type.STRING },
          },
          required: ["title", "scenario"],
      };
      // Classification/transcription task — DEFAULT_MODEL sufficient; no thinking needed
...
async generateThematicPlan(grade: Grade, topic: Topic, profile?: TeachingProfile, teacherHistoryContext?: string): Promise<AIGeneratedThematicPlan> {
      ...
      const prompt = `
### УЛОГА
Ти си експерт за курикулум по математика. Креирај ТЕМАТСКО ПЛАНИРАЊЕ (Прилог 1) за конкретна тема, идентично со официјалниот МОН образец.
...
- scenario: Детално сценарио за часот по официјалниот МОН формат (задолжително)
...
- scenario.main: Листа од 2-3 главни активности (~25 мин.) — КОНКРЕТНИ, именувани активности со чекор-по-чекор опис; вклучи mix од групна, парна и индивидуална работа; реални македонски контексти
- scenario.closing: Завршна активност (~5 мин.) — примена на наученото, задавање домашна работа
- scenario.reflection: Рефлексија — конкретна техника: "Математички семафор" / "ЗСН табела" / "3-2-1" / "Излезна картичка" / "Скала 1-5"
- scenario.homework: (опционално) Конкретна домашна работа — задачи со броеви страна/задача

ВАЖНО: Секоја активност во scenario.main мора да биде детално опишана (не само наслов). Вклучи реални примери (броеви, имиња, контексти).
`;
                  ...
                  items: {
                      ...
                      properties: {
                          ...
                          scenario: {
                              ...
                              required: ['intro', 'main', 'closing', 'reflection'],
                          },
                      },
                      required: ["lessonNumber", "lessonUnit", "learningOutcomes", "keyActivities", "assessment", "scenario"]
                  }
              }
          },
