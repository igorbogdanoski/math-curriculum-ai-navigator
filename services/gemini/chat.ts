...
async askTutor(message: string, history: Array<{ role: string; content: string }>, ragContext?: string, attemptCount?: number): Promise<string> {
...
7. Ниво на прашањето: за едноставни прашања од сеќавање (дефиниции, факти) дај директен, краток одговор. За прашања што бараат примена, анализа или повеќечекорно решавање, користи Сократовски пристап — прашувај наместо да кажуваш.
8. За терминологија претпочитај го стандардниот назив „коефициент на правец"; доколку ученикот употреби синоним како „наклон", прифати го без исправка — не инсистирај на едниот наспроти другиот.`;

    if (attemptCount !== undefined && attemptCount >= 3) {
      systemPrompt += `\n\n9. Ова е ${attemptCount}-ти обид на ученикот на истиот проблем во овој разговор. Премини на подиректно скелo (scaffolding): дај чекор-по-чекор насока со конкретни бројки/операции, но остави го ученикот сам да ја пресмета финалната бројка/одговор — сепак не го кажувај конечниот резултат наместо него.`;
    }
