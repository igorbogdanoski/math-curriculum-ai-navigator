...
/**
 * Uploads a student solution photo for the QR upload flow.
 * Returns the public download URL.
 * Path: solution-uploads/{token}/photo.{ext}
 */
export const uploadSolutionImage = async (file: File, token: string): Promise<string> => {
  if (file.size > MAX_IMAGE_BYTES) {
    throw new AppError(
      `Image size exceeds limit: ${(file.size / 1024 / 1024).toFixed(1)} MB`,
      ErrorCode.VALIDATION_FAILED,
  ...
  if (!ALLOWED_MIME_TYPES.has(file.type)) {
    throw new AppError(
      `Unsupported image format: ${file.type || 'unknown'}`,
      ErrorCode.VALIDATION_FAILED,
      'Неподдржан формат. Дозволени: JPEG, PNG, WebP.',
      false,
    );
  }
  const ext = file.name.split('.').pop()?.toLowerCase() || 'jpg';
  const path = `solution-uploads/${token}/photo.${ext}`;
  const storageRef = ref(storage, path);
